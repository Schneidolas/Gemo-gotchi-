    ...
    if (isDatingOrMarried) {
        switch(view) {
            ...
            case 'MENU_COMODOS':
                ...
                return (<> {rooms.map(room => ( <button key={room} className="action-btn" onClick={() => { setCurrentLocation(room); setView('HOME'); }}>{LOCATION_NAMES[room]}</button>))} <button className="action-btn wide-btn" onClick={() => setView('HOME')}>Voltar</button> </>);
